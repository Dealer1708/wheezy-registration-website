                ...
                if type_id != "region":
                    if last_parent_units["end_date"] != "":
                        ...
                        if check_unit_names[-1]['end_date'] == "" or check_class_codes[-1]['end_date'] == "":
                            error_list.append(f"In index {idx} parent uuid {parent_uuid} has end date but unit names and classifier codes don't")
            ...
            try:
                for p1 in parent_units:
                    ...
                    if parent_uuid not in unique_uuids:
                        error_list.append(f"In index {idx} parent uuid {parent_uuid} missing for the uuid {uuid}")
            except Exception as ex:
                error_list.append(f"In index {idx} parent uuid {parent_uuid} missing for the uuid {uuid}")
